...
def process_eog_to_json(file_obj):
    cols = [
        "Timestamp [us]",
        "QVAR [LSB]",
        "A_X [mg]",
...
def process_ecg_to_json(file_path):
...
def process_heart_to_json(file_path):
    ...
    times = np.arange(num_samples)  # Assuming 1Hz sampling rate

    return {
        "time": times.tolist(),
        "spo2": spo2[:num_samples].tolist(),
        "pr_bpm": pr[:num_samples].tolist()
    }
