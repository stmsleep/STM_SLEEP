  ...
  const [error, setError] = useState(null);
   ...
   useEffect(() => {
    const fetchAndUploadPDF = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get('http://localhost:8000/get_summary_pdf/');
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const simulatedData = {
        ...
        spo2Summary: [
          { range: "95-100", duration: spo2_summary['95-100'].Duration, total: spo2_summary['95-100']['%Total'], events: spo2_summary['95-100']['Events (ODI4%)']},
          { range: "89-94",duration: spo2_summary['89-94'].Duration, total: spo2_summary['89-94']['%Total'], events: spo2_summary['89-94']['Events (ODI4%)']},
          { range: "80-88",duration: spo2_summary["80-88"].Duration, total: spo2_summary["80-88"]['%Total'], events: spo2_summary["80-88"]['Events (ODI4%)']},
          { range: "70-79",duration: spo2_summary["70-79"].Duration, total: spo2_summary["70-79"]['%Total'], events: spo2_summary["70-79"]['Events (ODI4%)'] },
          { range: "<70",duration: spo2_summary["<70"].Duration, total: spo2_summary["<70"]['%Total'], events: spo2_summary["<70"]['Events (ODI4%)']},
          { range: "Total",duration: spo2_summary['Total'].Duration, total: spo2_summary['Total']['%Total'], events: spo2_summary['Total']['Events (ODI4%)']},
        ],
        metrics: {
          spo2: { max: spo2_pr_metrics.SpO2.max, avg: spo2_pr_metrics.SpO2.avg, min: spo2_pr_metrics.SpO2.min},
          pr: { max: spo2_pr_metrics.PR.max, avg: spo2_pr_metrics.PR.avg, min: spo2_pr_metrics.PR.min},
        },
        odi4: {
          ...
          percentageTimeInEvents: odi_info["ODI4%"]['% of Time in ODI 4% Events'],
        ...
        odi3: {
          ...
          percentageTimeInEvents: odi_info["ODI3%"]['% of Time in ODI 3% Events'],
          avgEventDuration: odi_info["ODI3%"]['Avg ODI 3% Event Duration'],
        },
        thresholdData: {
          threshold:thresholds['SpO2 Threshold'],
          spo2: [
            { threshold: thresholds["SpO2 >"][0], duration: thresholds["SpO2 >"][1], total: thresholds["SpO2 >"][2] },
            { threshold: thresholds["SpO2 <="][0], duration: thresholds["SpO2 <="][1], total: thresholds["SpO2 <="][2] },
          ],
          pulseRate: [
            { threshold: thresholds["PR >"][0], duration: thresholds["PR >"][1], total:thresholds["PR >"][2] },
            { threshold: thresholds["PR <"][0], duration: thresholds["PR <"][1], total: thresholds["PR <"][2]},
          ],
        },
        definitions: {
